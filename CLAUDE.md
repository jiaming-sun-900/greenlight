...
| `text-display` | 28px → 36px (fluid)   | `display-serif font-semibold`         | The page title only ("Screen a job posting", "Application Tracker"). One per view |
...
- **The display tier is the one serif in the app.** `display-serif` sets Newsreader, the face jiaming-sun-900.github.io is set in, and it belongs on the `h1` and nowhere else. Everything at `text-title` and below stays on Inter, deliberately: a serif goes mushy in a 12px chip, and Newsreader's figures are not tabular, which the column counts, the char count and the deadline all rely on. The utility also pins the optical-size axis to roughly the size the title renders at; left at the text master the strokes thin out and the title reads underweight beside Inter.
